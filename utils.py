...
class MaskAtom:
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        mask_pos_labels_list=[]
        for atom_idx in masked_atom_indices:
            mask_node_labels_list.append(data.x[atom_idx].view(1, -1))
            mask_pos_labels_list.append(data.pos[atom_idx].view(1,-1))
        data.mask_node_label = torch.cat(mask_node_labels_list, dim=0)
        data.masked_atom_indices = torch.tensor(masked_atom_indices)
        data.pos_label = torch.cat(mask_pos_labels_list, dim=0)
        # ----------- graphMAE -----------
        ...
        for atom_idx in masked_atom_indices:
            ...
            data.pos[atom_idx] = torch.tensor([1,1,1])
        if self.mask_edge:
            ...
            data.edge_attr_label = data.mask_edge_label
